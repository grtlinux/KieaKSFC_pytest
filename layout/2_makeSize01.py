...
# ----------------------------------------------
def get_length(lst: list=None) -> int:
  ''' get_length '''
  len = 0
  for itm in lst:
    if itm['type'] == 'ARROBJ':
      len += get_length(itm['arrFields']) * itm['arrMax']
    else:
      len += itm['size']
  return len

# ----------------------------------------------
def get_reqres_length(file_name) -> None:
  ''' get_reqres_length '''
...
if __name__ == '__main__':
  # get_reqres_length('/content/drive/MyDrive/json/trx_001.json')
